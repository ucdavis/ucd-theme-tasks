...
export default async function init (parentPath, options) {
  ...
  const copyOptions = {
    filter: (src) => {
      if (options.themesync || options.patternlab) return true
      // Exclude tasks-config.js
      return !src.endsWith('tasks-config.js')
    },
  ...
  // Copy root files.
  try {
    await fs.copy(`${import.meta.dirname}/../starter/`, dest, copyOptions)
  } catch (error) {
